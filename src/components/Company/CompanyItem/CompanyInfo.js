...
import CompanyContainRoom from './CompanyContainRoom';
...
export class CompanyInfo extends Component {
  ...
  // getRoomOfCompany = (contract, id) => {
  //   var result = [];
  //   console.log("id company: ", id);
  //   // eslint-disable-next-line array-callback-return
  //   for (let i = 0; i < contract.length; i++) {
  //     console.log("companyId in contract: ", contract[i].company.id);
  //     if (contract[i].company.id == id)
  ...
  componentDidMount() {
    ...
    // console.log("result: " + this.getRoomOfCompany(this.props.contract, match.params.id))
    if (match) {
      // lay tham so url
  ...
  showRoomCompany = (contract, id) => {
    var result = null;
    result = contract.map((item, index) => {
      if (item.company.id == id)
        return <CompanyContainRoom key={index} companyContainRoom={item} index={index} />
    })
    return result;
  }
        ...
        <div className='col-xs-8 col-sm-8 col-md-8 col-lg-8'>
          ...
          <div className="panel-body">
            <table className="table table-hover">
              ...
              <tbody>
                {this.showRoomCompany(contract, this.props.match.params.id)}
              </tbody>
            </table>
          </div>
